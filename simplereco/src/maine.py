...
def preprocess(img, imgSize, dataAugmentation=False):
	...
	return img
#img = cv2.imread('tesst.png', cv2.IMREAD_GRAYSCALE)
#preprocess(img, (128,32), dataAugmentation=False)
    ...
    while True:
        try:
            ...
            if key == ord('s'):
                ...
                img_new = img_new[60:420,0:640]
                print(len(img_new),len(img_new[0]))
                ...
                img_ = img_[60:420,0:640]
                cv2.imshow("saving",img_)
                #cv2.waitKey(1000)
                img_ = cv2.resize(gray,(173,93))
                print("Resized...")
                img_ = img_[13:80,0:173]
                cv2.imshow("saving",img_)

                img_resized = cv2.imwrite(filename='tesst.png', img=img_)
                #cv2.waitKey(10000)
                print("Image saved!")
                break
            elif key == ord('q'):
...
class FilePaths:
	...
	fnTrain = '../data/'
	fnInfer = 'uploaded_images/tesst.png'#'../data/test.png' #
